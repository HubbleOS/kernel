...
// #define TEST_VIRT 0x0000000010000000ULL // Безопасная область для теста
// #define TEST_PAGES 2

// void vmm_test(void)
// {
// 	printf("=== VMM Test Start ===\n");
...
// 			return;
// 		}
// 		phys[i] = pmm_get_phys(virt);
// 		printf("Allocated physical frame %d at phys=0x%llx\n", i, phys[i]);
// 	}

// 	// 2. Маппим в тестовую область
// 	for (int i = 0; i < TEST_PAGES; i++)
// 	{
// 		if (vmm_map(TEST_VIRT + i * PAGE_SIZE, phys[i], 1, PTE_WRITABLE) != 0)
// 		{
// 			printf("Failed to map page %d\n", i);
// 			return;
// 		}
// 	}
// 	printf("Mapped %d pages at virtual 0x%llx\n", TEST_PAGES, TEST_VIRT);
...
// 		printf("Translate: 0x%llx -> 0x%llx (expected 0x%llx) %s\n",
...
// 		ptr[0] = 0xAA + i;
// 		ptr[1] = 0xBB + i;
// 		printf("Wrote to virt page %d: [0x%02x, 0x%02x]\n", i, ptr[0], ptr[1]);
// 	}
...
// 	{
// 		uint8_t *ptr = (uint8_t *)(TEST_VIRT + i * PAGE_SIZE);
// 		printf("Read from virt page %d: [0x%02x, 0x%02x]\n", i, ptr[0], ptr[1]);
// 	}

// 	// 6. Unmap
// 	vmm_unmap(TEST_VIRT, TEST_PAGES);
// 	printf("Unmapped test pages\n");

// 	// 7. Освобождение (через виртуальные адреса из pmm_alloc)
// 	// Примечание: нужно сохранить virt указатели, но здесь упрощённо
// 	printf("=== VMM Test End ===\n");
// }
