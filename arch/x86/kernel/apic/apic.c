...
void lapic_timer_init(uint32_t frequency_hz) {
  ...
  if (elapsed == 0 || elapsed < 1000) {
    printk(KERN_ERR
           "ERROR: LAPIC timer calibration failed (elapsed too small)\n");
    return;
  }
...
void apic_debug_check(void) {
  ...
  volatile uint32_t *direct =
      (volatile uint32_t *)(0xFFFF800000000000ULL + (uint64_t)lapic_phys);
 ...
 */
static void setup_iso_callback(uint8_t irq_source, uint32_t gsi, uint16_t flags,
                               void *ctx) {
  (void)ctx;
...
int apic_init(void) {
  ...
  printk(KERN_INFO "I/O APIC version: 0x%x, max redirects: %u\n", ver & 0xFF,
         apic_state.ioapic_max_redirect);
 ...
 */
bool apic_is_initialized(void) { return apic_state.initialized; }
