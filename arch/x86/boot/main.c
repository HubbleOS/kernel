...
static void draw_pixel_array_scaled(const uint8_t *glyph, int pitch,
...
extern void
jump_to_kernel(void *boot_info, void *entry, uint64_t stack);
				      ...
				      uint64_t *next_free)
{
	for (uint64_t i = 0; i < ctx->mem_map_count; i++)
	{
		...
		map_phys_range_2mb(pml4, d->phys_start, d->num_pages * 0x1000,
				   next_free);
	}

	map_phys_range_2mb(pml4, (uint64_t)ctx, sizeof(*ctx), next_free);
	map_phys_range_2mb(pml4, (uint64_t)ctx->mem_map,
			   ctx->mem_map_count * sizeof(ctx->mem_map[0]),
			   next_free);
	map_phys_range_2mb(pml4, (uint64_t)ctx->elf_buf, ctx->elf_size,
			   next_free);

	uint64_t fb_size = (uint64_t)ctx->framebuffer.pitch *
			   ctx->framebuffer.height;
	map_phys_range_2mb(pml4, (uint64_t)ctx->framebuffer.base, fb_size,
			   next_free);
}
...
void boot_main(loader_context_t *ctx)
{
	...
	/* 1. Kernel: KERNEL_VIRT_BASE -> KERNEL_PHYS_BASE
	...
	/* 3. Identity and direct-map all firmware-reported physical ranges.
	      Real machines commonly place the loader, stack, free RAM, or GOP
	      framebuffer above 4GB. */
	map_loader_context_ranges(pml4, ctx, &pt_arena);

	uint64_t arena_reserved = min_u64(free_size - (arena_start - free_base),
					  64 * 1024 * 1024);
