...
void fat32_format_directory_cluster(FAT32_FS *fs, uint32_t cluster,
  ...
  FAT32_DirectoryEntry *dotdot =
      (FAT32_DirectoryEntry *)(buf + sizeof(FAT32_DirectoryEntry));
...
bool fat32_create_directory(FAT32_FS *fs, const char *path) {
  ...
  return fat32_create_entry(fs, dir_cluster, &parts.parts[parts.count - 1],
                            true);
}
...
bool fat32_delete_directory(FAT32_FS *fs, const char *path) {
  ...
  uint32_t dir_cluster = find_directory_entry_cluster(
      fs, parent_cluster, parts.parts[parts.count - 1].sfn);
