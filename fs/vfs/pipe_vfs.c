...
static VFS_pipes_node *pipe_vfs_find_pipe_node(VFS_pipes_tree *pipe_tree,
                                               const char *name) {
...
VFS_Node *pipe_vfs_open_pipe(VFS_FS *fs, const char *path) {
  ...
  parts.count++;
  if (pipe) {
    VFS_pipes_node *pipe_node =
        pipe_vfs_find_pipe_node(pipe, parts.parts[parts.count - 1].name);
    if (pipe_node) {
