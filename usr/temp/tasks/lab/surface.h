...
surface_t *surface_create(int nx, int ny, float x0, float x1, float y0, float y1);
...
/* Draw the surface with floating-horizon hidden-line removal.
   ...
   fill_cell_fn         – pointer to your canvas fill-quad function (may be NULL).
   userdata             – passed through to both callbacks.
   wire_color           – colour for wireframe lines.
   fill_color           – colour for cell fill (ignored if fill_cell_fn == NULL).
   bg_color             – background colour used to "erase" hidden lines.     */
typedef void (*draw_line_fn)(void *ud, int x0, int y0, int x1, int y1, unsigned color);
typedef void (*fill_quad_fn)(void *ud, int x[4], int y[4], unsigned color);

void surface_draw(const surface_t *s,
		  draw_line_fn draw_line,
		  fill_quad_fn fill_quad,
		  void *userdata,
		  unsigned wire_color,
		  unsigned fill_color,
		  int canvas_w);
...
void surface_draw_axes(const proj_t *p,
		       float len,
		       draw_line_fn draw_line,
		       void *userdata,
		       unsigned color);
