...
// regular coordinates
// Operations are applied directly to the current points, using formulas without
// matrices.
...
static void apply_matrix(transform_object_t *obj, float a, float b, float tx,
...
// | 0  1  dy |
// | 0  0   1 |
void transform_translate_homogeneous(transform_object_t *obj, float dx,
                                     float dy) {
  apply_matrix(obj, 1, 0, dx, 0, 1, dy);
  obj->offset_x += dx;
  obj->offset_y += dy;
}
...
void transform_scale_homogeneous(transform_object_t *obj, float sx, float sy) {
  ...
  apply_matrix(obj, sx, 0, cx * (1.0f - sx), 0, sy, cy * (1.0f - sy));
  obj->scale_x *= sx;
  obj->scale_y *= sy;
}
...
void transform_rotate_homogeneous(transform_object_t *obj, float angle_deg) {
  ...
  apply_matrix(obj, cosA, -sinA, cx - cx * cosA + cy * sinA, sinA, cosA,
               cy - cx * sinA - cy * cosA);
  obj->angle += angle_deg;
}
