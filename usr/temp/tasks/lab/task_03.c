/*
 * Geometry Tasks Application
 *
 * Task 1 – Point symmetric to a given point w.r.t. a centre of symmetry
 * Task 2 – Point symmetric to a given point w.r.t. an axis of symmetry
 * Task 3 – Polygon convexity check + traversal direction
 * Task 4 – Convex hull (Graham scan) for a set of points
 */
...
static input_t *t2_ad;	       /* axis offset (d)  */
...
/*
 *  TASK 1 – Symmetric point w.r.t. centre of symmetry
 *
 *  P' = 2·C – P
 */
static void task1_draw(void)
{
	...
	canvas_draw_line(g_cnv, wx(px), wy(py), wx(rx), wy(ry),
			 rgb(120, 120, 180));

	/* Centre crosshair */
	int ccx = wx(cx), ccy = wy(cy);
	draw_circle(g_cnv, ccx, ccy, 5, COL_CENTRE);

	/* Source point P */
	draw_circle(g_cnv, wx(px), wy(py), 6, COL_POINT_SRC);

	/* Result point P' */
	draw_circle(g_cnv, wx(rx), wy(ry), 6, COL_POINT_DST);

	/* Small legend */
	char buf[64];
	snprintf(buf, sizeof(buf), "P(%.0f,%.0f)", px, py);
	/* labels are approximate — just draw tiny markers */
	// draw_border(g_cnv);
}

/*
 *  TASK 2 – Symmetric point w.r.t. an axis (line ax+by+d=0)
 *
 *  Axis defined by direction vector (ax, ay) and offset d
 *  Normal form:  (-ay)·x + (ax)·y + d = 0
 *                A = -ay, B = ax
 *
 *  P' = P - 2·(A·px + B·py + d)/(A²+B²) · (A, B)
 */
static void task2_draw(void)
{
	float px = atof(t2_px->text), py = atof(t2_py->text);
	float ax = atof(t2_ax->text), ay = atof(t2_ay->text);
	float d = atof(t2_ad->text);
	...
	canvas_draw_line(g_cnv, ax1, ay1, ax2, ay2, COL_AXIS);

	canvas_draw_line(g_cnv, wx(px), wy(py), wx(rx), wy(ry), rgb(120, 120, 180));

	/* Foot of perpendicular */
	draw_circle(g_cnv, wx(fx), wy(fy), 4, COL_CENTRE);
...
/*
 *  TASK 3 – Polygon convexity + traversal direction
 *
 *  Cross product sign of consecutive edge pairs:
 *   all positive → CCW convex
 *   all negative → CW  convex
 *   mixed        → concave
 */
static void task3_draw(void)
{
	...
	for (int i = 0; i < n; i++)
	{
		...
		draw_circle(g_cnv, wx(pts[i].x), wy(pts[i].y), 5, col);
	}
	...
	for (int i = 0; i < n; i++)
	{
		...
		float cross = (pts[b].x - pts[a].x) * (pts[c].y - pts[b].y) - (pts[b].y - pts[a].y) * (pts[c].x - pts[b].x);
		if (cross > 0)
			pos++;
		else if (cross < 0)
			neg++;
	...
	if (pos > 0 && neg > 0)
		label_set_text(t3_result_lbl, "CONCAVE (non-convex)");
	else if (pos > 0)
		label_set_text(t3_result_lbl, "CONVEX — CCW (counter-clockwise)");
	else if (neg > 0)
		label_set_text(t3_result_lbl, "CONVEX — CW (clockwise)");
	else
...
static float cross2(pt2_t O, pt2_t A, pt2_t B)
{
...
static int convex_hull(pt2_t *pts, int n, pt2_t *hull)
{
	...
	{
		while (k >= t && cross2(hull[k - 2], hull[k - 1], pts[i]) <= 0)
			k--;
		hull[k++] = pts[i];
	}
	return k - 1;
...
static void task4_draw(void)
{
	...
	/* All input points */
	for (int i = 0; i < n; i++)
		draw_circle(g_cnv, wx(pts[i].x), wy(pts[i].y), 3, COL_INNER_PT);
	...
	for (int i = 0; i < hn; i++)
	{
		...
		int mx = (wx(hull[i].x) + wx(hull[j].x)) / 2;
		int my = (wy(hull[i].y) + wy(hull[j].y)) / 2;
	}

	/* Hull vertices */
	for (int i = 0; i < hn; i++)
		draw_circle(g_cnv, wx(hull[i].x), wy(hull[i].y), 3, COL_HULL_PT);
...
/* We group panels by hiding / showing them.
   Since the GUI has no explicit show/hide, we use separate panel
   objects built at init time and placed off-screen when inactive.
   Instead, we simply rebuild inputs per task. For simplicity in
   this framework we keep all inputs always present but place
   inactive ones off-screen at x=-2000. */

/* ─── panel element structs ─── */
/* Task-1 panel widgets */
static label_t *t1_lbl_px, *t1_lbl_py, *t1_lbl_cx, *t1_lbl_cy;
static button_t *t1_btn;

/* Task-2 panel widgets */
static label_t *t2_lbl_px, *t2_lbl_py;
static label_t *t2_lbl_ax, *t2_lbl_ay, *t2_lbl_d;
static button_t *t2_btn;

/* Task-3 panel widgets */
static label_t *t3_lbl_pts;
static button_t *t3_btn;

/* Task-4 panel widgets */
...
{
	/* Main window: 800×580 */
	g_win = window_create(60, 40, 800, 580);

	/* Drawing canvas */
	g_cnv = canvas_create(CNV_X, CNV_Y, CNV_W, CNV_H);
	window_addElement(g_win, g_cnv);

	/* ── Menu buttons (top of side panel) ── */
	...
	for (int i = 0; i < 4; i++)
	{
		...
		window_addElement(g_win, menu_btn[i]);
	}
	...
	window_addElement(g_win, t1_lbl_px);
	window_addElement(g_win, t1_px);
	window_addElement(g_win, t1_lbl_py);
	window_addElement(g_win, t1_py);
	...
	window_addElement(g_win, t1_lbl_cx);
	window_addElement(g_win, t1_cx);
	window_addElement(g_win, t1_lbl_cy);
	window_addElement(g_win, t1_cy);

	py += 32;
	t1_btn = button_create(PANEL_X, py, 175, 28, "Apply");
	t1_btn->on_click = on_apply;
	window_addElement(g_win, t1_btn);

	/*
	 *  Task-2 panel  (placed just below task-1)
	 */
	py += 40;
	...
	window_addElement(g_win, t2_lbl_px);
	window_addElement(g_win, t2_px);
	window_addElement(g_win, t2_lbl_py);
	window_addElement(g_win, t2_py);
	...
	t2_lbl_d = label_create(PANEL_X + 164, py, 12, 20, "d:");
	t2_ad = input_create(PANEL_X + 178, py, 35, 22, "0");
	window_addElement(g_win, t2_lbl_ax);
	window_addElement(g_win, t2_ax);
	window_addElement(g_win, t2_lbl_ay);
	window_addElement(g_win, t2_ay);
	window_addElement(g_win, t2_lbl_d);
	window_addElement(g_win, t2_ad);

	py += 32;
	t2_btn = button_create(PANEL_X, py, 175, 28, "Apply");
	t2_btn->on_click = on_apply;
	window_addElement(g_win, t2_btn);
	...
	window_addElement(g_win, t3_lbl_pts);
	py += 22;
	t3_pts_input = input_create(PANEL_X, py, 175, 22,
				    "0,80;80,-40;-80,-40");
	window_addElement(g_win, t3_pts_input);
	py += 28;
	t3_btn = button_create(PANEL_X, py, 175, 28, "Check");
	t3_btn->on_click = on_apply;
	window_addElement(g_win, t3_btn);
	py += 32;
	t3_result_lbl = label_create(PANEL_X, py, 175, 22, "");
	window_addElement(g_win, t3_result_lbl);
	...
	window_addElement(g_win, t4_lbl_pts);
	py += 22;
	t4_pts_input = input_create(PANEL_X - 75, py, 175 + 75, 22, "9,9;-9,9;9,-9;-9,-9");
	window_addElement(g_win, t4_pts_input);
	py += 28;
	t4_btn = button_create(PANEL_X, py, 175, 28, "Build Hull");
	t4_btn->on_click = on_apply;
	window_addElement(g_win, t4_btn);
}
