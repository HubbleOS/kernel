...
static const point_t SQUARE_PTS[] = {
    {-50, -50}, {50, -50}, {50, 50}, {-50, 50}};
static const point_t HEXAGON_PTS[] = {{0, -50}, {43, -25}, {43, 25},
                                      {0, 50},  {-43, 25}, {-43, -25}};
...
static void geo_draw(canvas_t *cnv, transform_object_t *obj, int ox, int oy) {
  ...
  uint32_t color =
      rgb((int)(obj->color_r * 255.0f), (int)(obj->color_g * 255.0f),
...
static void set_shape(const point_t *pts, size_t count, float r, float g,
                      float b) {
  if (g_obj)
...
// callbacks
static void on_triangle() { set_shape(TRIANGLE_PTS, 3, 0.0f, 0.8f, 1.0f); }

static void on_square() { set_shape(SQUARE_PTS, 4, 1.0f, 0.8f, 0.0f); }

static void on_hexagon() { set_shape(HEXAGON_PTS, 6, 0.8f, 0.0f, 1.0f); }
