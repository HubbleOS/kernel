...
static const point3d_t TETRA_VERTS[] = {
    ...
    {-23.094f, -40.0f, 0.0f},
    {0.0f, 0.0f, 65.28f}};
static const edge_t TETRA_EDGES[] = {
...
static const point3d_t STAR_VERTS[] = {
    {-50, -50, -50}, {50, -50, -50}, {50, 50, -50}, {-50, 50, -50}, {-50, -50, 50}, {50, -50, 50}, {50, 50, 50}, {-50, 50, 50}, {0, 0, -120}, {0, 0, 120}, {0, 120, 0}, {0, -120, 0}, {120, 0, 0}, {-120, 0, 0}};
static const edge_t STAR_EDGES[] = {
    {0, 1}, {1, 2}, {2, 3}, {3, 0}, {4, 5}, {5, 6}, {6, 7}, {7, 4}, {0, 4}, {1, 5}, {2, 6}, {3, 7}, {8, 0}, {8, 1}, {8, 2}, {8, 3}, {9, 4}, {9, 5}, {9, 6}, {9, 7}, {10, 2}, {10, 3}, {10, 6}, {10, 7}, {11, 0}, {11, 1}, {11, 4}, {11, 5}, {12, 1}, {12, 2}, {12, 5}, {12, 6}, {13, 0}, {13, 3}, {13, 4}, {13, 7}};
...
static const point3d_t HUMAN_VERTS[] = {
    {-10, -5, 0}, {10, -5, 0}, {-10, 5, 0}, {10, 5, 0}, {-15, -8, 60}, {15, -8, 60}, {-15, 8, 60}, {15, 8, 60}, {-7, -7, 85}, {7, -7, 85}, {-7, 7, 85}, {7, 7, 85}, {0, 0, 110}, {-25, -5, 35}, {-30, 0, 10}, {25, -5, 35}, {30, 0, 10}, {-8, -3, -40}, {-8, 0, -75}, {8, -3, -40}, {8, 0, -75}, {0, 0, 30}, {0, 0, 75}};
static const edge_t HUMAN_EDGES[] = {
    {0, 1}, {1, 3}, {3, 2}, {2, 0}, {4, 5}, {5, 7}, {7, 6}, {6, 4}, {0, 4}, {1, 5}, {2, 6}, {3, 7}, {8, 9}, {9, 11}, {11, 10}, {10, 8}, {8, 12}, {9, 12}, {10, 12}, {11, 12}, {22, 8}, {22, 9}, {22, 10}, {22, 11}, {4, 13}, {6, 13}, {13, 14}, {5, 15}, {7, 15}, {15, 16}, {0, 17}, {2, 17}, {17, 18}, {1, 19}, {3, 19}, {19, 20}, {21, 22}, {21, 0}};
 ...
 * Edges that belong to NO face (wire-only shapes like Human) are always visible.
 */
...
/*
   Drawing
 */
static void poly_draw(canvas_t *cnv, const polyhedron3d_t *poly,
		      int ox, int oy)
{
	if (!cnv || !poly)
		return;

	uint32_t edge_color = rgb((int)(poly->color_r * 255.0f),
	...
	for (size_t i = 0; i < poly->edge_count; i++)
	{
		...
		canvas_draw_line(cnv, ox + pa.x, oy + pa.y,
				 ox + pb.x, oy + pb.y, edge_color);
	}
	free(mask);
...
static polyhedron3d_t *make_poly(
    const point3d_t *verts, size_t nv,
    const edge_t *edges, size_t ne,
    const face_t *faces, size_t nf,
...
static void on_cube(void)
{
	set_poly(make_poly(CUBE_VERTS, ARRAY_SIZE(CUBE_VERTS),
			   ...
			   CUBE_FACES, ARRAY_SIZE(CUBE_FACES),
			   0.2f, 0.8f, 1.0f));
}
static void on_tetrahedron(void)
{
	set_poly(make_poly(TETRA_VERTS, ARRAY_SIZE(TETRA_VERTS),
			   ...
			   TETRA_FACES, ARRAY_SIZE(TETRA_FACES),
			   1.0f, 0.2f, 0.8f));
}
static void on_octahedron(void)
{
	set_poly(make_poly(OCTA_VERTS, ARRAY_SIZE(OCTA_VERTS),
			   ...
			   OCTA_FACES, ARRAY_SIZE(OCTA_FACES),
			   0.8f, 1.0f, 0.2f));
}
static void on_star(void)
{
	set_poly(make_poly(STAR_VERTS, ARRAY_SIZE(STAR_VERTS),
			   ...
			   STAR_FACES, ARRAY_SIZE(STAR_FACES),
			   1.0f, 0.9f, 0.2f));
}
...
static void redraw(void)
{
	...
	canvas_draw_line(g_cnv, 0, g_cnv->height - 1,
			 g_cnv->width - 1, g_cnv->height - 1, rgb(60, 60, 70));
