...
/*
   Allocation
    */
surface_t *surface_create(int nx, int ny,
			  float x0, float x1,
			  float y0, float y1)
{
	if (nx < 2 || ny < 2)
...
static void horizon_update(int *top, int *bot, int w,
...
static void horizon_draw_seg(int *top, int *bot, int w,
			     int x0, int y0, int x1, int y1,
			     draw_line_fn draw, void *ud, unsigned color)
{
		  ...
		  int canvas_w)
{
	...
	for (int iy = 0; iy < s->ny - 1; iy++)
	{
		for (int ix = 0; ix < s->nx - 1; ix++)
		{
			...
			/* top edge (iy row) */
			horizon_draw_seg(top, bot, canvas_w,
					 p00.x, p00.y, p10.x, p10.y,
					 draw_line, userdata, wire_color);
			horizon_update(top, bot, canvas_w,
				       p00.x, p00.y, p10.x, p10.y);
...
/*
   Axes
    */
void surface_draw_axes(const proj_t *p, float len,
		       draw_line_fn draw_line, void *ud,
		       unsigned color)
{
	pt3_t o = {0, 0, 0};
	pt3_t xa = {len, 0, 0};
