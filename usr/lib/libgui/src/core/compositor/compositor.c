    ...
    for (int y = r.y; y < r.y + r.h; y++) {
      ...
      for (int l = 0; l < MAX_LAYERS; l++) {
        ...
        for (int i = 0; i < layer->count; i++) {
          ...
          int ix2 = (obj->x + obj->width) < (r.x + r.w) ? (obj->x + obj->width)
                                                        : (r.x + r.w);
          if (ix1 >= ix2)
            continue;

          uint32_t *src =
              obj->buffer + (y - obj->y) * obj->width + (ix1 - obj->x);
...
void compositor_render() { compositor_frame(); }
...
void compositor_change_size_object(object_t *obj, int new_w, int new_h) {
  ...
  for (int y = 0; y < copy_h; y++)
    memcpy(new_buf + y * new_w, obj->buffer + y * obj->width,
