...
{
	return el->dirty_rect.valid &&
	       el->dirty_rect.x == 0 && el->dirty_rect.y == 0 &&
	       el->dirty_rect.w >= el->width && el->dirty_rect.h >= el->height;
}
...
void object_flush(object_t *obj)
{
	for (int e = 0; e < obj->element_count; e++)
	{
		...
		for (int j = e + 1; j < obj->element_count; j++)
		{
			...
			int ix2 = (over->x + over->width) < (abs_x + dw) ? (over->x + over->width) : (abs_x + dw);
			int iy2 = (over->y + over->height) < (abs_y + dh) ? (over->y + over->height) : (abs_y + dh);
...
void object_move_element(object_t *obj, element_t *el, int new_x, int new_y)
{
	...
	for (int e = 0; e < obj->element_count; e++)
	{
		...
		int ix2 = (other->x + other->width) < (ox + ow) ? (other->x + other->width) : (ox + ow);
		int iy2 = (other->y + other->height) < (oy + oh) ? (other->y + other->height) : (oy + oh);
