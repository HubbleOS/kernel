...
static void e1000_rx_init(void) {
  rx_descs = kmalloc(sizeof(struct e1000_rx_desc) * E1000_RX_DESC_COUNT + 16,
                     GFP_KERNEL);
...
static void e1000_tx_init(void) {
  tx_descs = kmalloc(sizeof(struct e1000_tx_desc) * E1000_TX_DESC_COUNT + 16,
                     GFP_KERNEL);
...
int e1000_send(const void *data, uint16_t len) {
  ...
  printk(KERN_INFO "[tx] TCTL=%08x TDBAL=%08x TDBAH=%08x TDLEN=%08x\n",
         e1000_read(E1000_TCTL), e1000_read(E1000_TDBAL),
         e1000_read(E1000_TDBAH), e1000_read(E1000_TDLEN));
...
int e1000_recv(void *buf, uint16_t *len_out) {
  ...
  if (!first) {
    first = 1;
    printk(KERN_INFO
           "[rx] FIRST CALL: idx=%d status=%02x RDH=%d RDT=%d RDBAL=%08x\n",
           idx, rx_descs[idx].status, e1000_read(E1000_RDH),
           e1000_read(E1000_RDT), e1000_read(E1000_RDBAL));
  }
...
static int e1000_probe(struct pci_device *pci_dev) {
  ...
  pci_cmd |= (1 << 2) | (1 << 1);
  pci_write_config(e1000_pci_bus, e1000_pci_slot, e1000_pci_func, 0x04,
                   pci_cmd);
