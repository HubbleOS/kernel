/**
 * @file uhci-hcd.c
 * @brief UHCI host controller driver — reset, frame list, port management,
 *        control/interrupt transfers, PCI probe
 */
#include "uhci-hcd.h"
#include "uhci-td.h"
...
static int uhci_get_device_descriptor(struct uhci_hcd *hcd, uint8_t dev_addr,
                                      int is_low_speed);
static int uhci_set_configuration(struct uhci_hcd *hcd, uint8_t dev_addr,
                                  uint8_t config);
...
static int uhci_set_address(struct uhci_hcd *hcd, uint8_t new_addr) {
  ...
  struct usb_setup_packet *pkt =
      (struct usb_setup_packet *)phys_to_virt(setup_phys);
  ...
  printk(KERN_INFO
         "[uhci] set_addr: td_phys=0x%llx td[0].link=0x%x td[1] addr=0x%llx\n",
...
static int uhci_get_device_descriptor(struct uhci_hcd *hcd, uint8_t dev_addr,
  ...
  struct usb_device_descriptor *desc =
      (struct usb_device_descriptor *)phys_to_virt(buf_phys);
...
static int uhci_get_config_descriptor(struct uhci_hcd *hcd, uint8_t dev_addr,
                                      int is_low_speed, uint8_t *out_protocol,
  ...
  pkt->bmRequestType = 0x80;
  pkt->bRequest = 0x06;
  pkt->wValue = (USB_DESC_CONFIGURATION << 8);
  pkt->wIndex = 0;
  ...
  *out_max_packet = 8;
  ...
  while (p < end && p[0] > 0) {
    ...
    if (type == USB_DESC_INTERFACE) {
      ...
      printk(KERN_INFO
             "[uhci] interface: class=0x%02x subclass=0x%02x proto=0x%02x\n",
      ...
      struct usb_endpoint_descriptor *ep = (struct usb_endpoint_descriptor *)p;
...
/* -- Keyboard interrupt transfer -------------------------- */

static int uhci_keyboard_transfer(struct uhci_hcd *hcd, uint8_t dev_addr,
                                  int is_low_speed) {
  uint32_t ls = is_low_speed ? TD_STATUS_LS : 0;
  uint64_t buf_phys = pmm_alloc_page();
  uint8_t *buf = (uint8_t *)phys_to_virt(buf_phys);
  ...
  while (1) {
    ...
    td->token = TD_TOKEN(TD_PID_IN, dev_addr, 1, toggle, KBD_REPORT_SIZE - 1);
...
static int uhci_interrupt_transfer(struct uhci_hcd *hcd, uint8_t dev_addr) {
  ...
  while (1) {
    ...
    td->token = TD_TOKEN(TD_PID_IN, dev_addr, 1, toggle, MOUSE_REPORT_SIZE - 1);
    td->buffer = (uint32_t)buf_phys;
...
static void uhci_check_ports(struct uhci_hcd *hcd) {
  for (int i = 0; i < hcd->num_ports; i++) {
    ...
    if (portsc & PORTSC_CS) {
      ...
      if (uhci_get_config_descriptor(hcd, i + 1, is_low_speed, &protocol,
                                     &endpoint, &max_packet) < 0) {
