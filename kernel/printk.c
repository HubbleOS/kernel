...
static const struct printk_token tokens[] = {
    {"[ OK ]", 6, COLOR_GREEN}, {"[OK]", 4, COLOR_GREEN},
    {"[ERROR]", 7, COLOR_RED},  {"[WARN]", 6, COLOR_YELLOW},
    {"[INFO]", 6, COLOR_BLUE},  {"[DEBUG]", 7, COLOR_WHITE},
};
...
size_t printk_log_size(void) { return log_size; }
...
static const char *parse_precision(const char *fmt, int *precision,
                                   va_list *args) {
  if (*fmt != '.') {
    *precision = -1;
    return fmt;
...
static char *format_uint(uintmax_t num, char *buf, int base, bool uppercase,
                         int precision) {
  ...
  while (*fmt) {
    ...
    case 'i':
      ...
      case LEN_LL:
        ...
        break;
      case LEN_Z:
        ival = (intmax_t)va_arg(args_copy, size_t);
        break;
      case LEN_T:
        ival = (intmax_t)va_arg(args_copy, ptrdiff_t);
        break;
      case LEN_J:
    ...
    case 'X':
      ...
      case LEN_Z:
        ...
        break;
      case LEN_T:
        uval = (uintmax_t)va_arg(args_copy, ptrdiff_t);
        break;
      case LEN_J:
