 ...
 *
 * Implements loading of ELF relocatable object files (.ko) into kernel
 * memory, resolving cross-module and kernel-exported symbols, running
 * initialisation and exit functions, and tracking loaded modules in a
 * linked list for unload and symbol resolution.
 */
...
/**
 * @brief Entry in the kernel's symbol table exported to loadable modules.
 */
typedef struct {
	const char *name;
	uint64_t    addr;
...
/**
 * @brief Describes one section of a module ELF image mapped into memory.
 */
typedef struct {
	const char *name;
	uint64_t    base;
...
/**
 * @brief One exported symbol from a loaded module.
 */
typedef struct {
	char     name[64];
	uint64_t addr;
...
/**
 * @brief Section metadata stored per loaded module.
 */
typedef struct {
	char     name[64];
	uint64_t base;
...
/**
 * @brief Per-module bookkeeping structure.
 */
struct module {
	char                    name[64];
	char                    path[256];
...
static const char *module_section_name(const char *shstrtab,
...
/**
 * @brief Look up a symbol in the kernel's export table.
 */
...
/**
 * @brief Look up a symbol across all loaded modules' export tables.
 */
static uint64_t module_resolve_module_export(const char *name)
{
	for (module_t *cur = g_modules_list; cur; cur = cur->next)
	{
...
/**
 * @brief Resolve a symbol name to its address.
 *
 * Checks kernel exports first, then iterates loaded module exports.
 */
...
/**
 * @brief Resolve an ELF symbol to a runtime address.
 *
 * Handles undefined (external), absolute, common, and section-relative
 * symbols. External symbols are resolved via module_resolve_export().
 */
...
/**
 * @brief Apply a single ELF64 relocation.
 *
				   ...
				   size_t section_count)
{
	...
	switch (type)
	{
	case R_X86_64_64:   patch_size = 8;  break;
	case R_X86_64_PC32:
	case R_X86_64_PLT32:
	case R_X86_64_32:
	case R_X86_64_32S:  patch_size = 4;  break;
	case R_X86_64_16:   patch_size = 2;  break;
	case R_X86_64_8:    patch_size = 1;  break;
	default:            patch_size = 0;  break;
	}
	...
	if (rela->r_offset > target_size ||
	    target_size - rela->r_offset < patch_size)
		return -ENOEXEC;
	...
	case R_X86_64_64:
	{
		uint64_t value = symbol + rela->r_addend;
	...
	case R_X86_64_PLT32:
	{
		int64_t  reloc = (int64_t)symbol + rela->r_addend -
...
/**
 * @brief Map a region of virtual memory for a module section.
 *
 * Allocates physical pages and maps them into the kernel's higher-half
 * region at a monotonically increasing virtual address.
 *
 * @param size          Size of the region.
 * @param sh_flags      ELF section flags (SHF_WRITE, SHF_EXECINSTR, …).
 * @param sh_addralign  Alignment requirement from the ELF section header.
 * @return Virtual base address, or 0 on failure.
 */
static uint64_t module_map_region(size_t size, uint64_t sh_flags,
				  uint64_t sh_addralign)
{
...
/**
 * @brief Unmap a previously mapped module region and free its pages.
 */
 ...
 *
 * @param name Case-insensitive module name.
 * @return Pointer to the module, or NULL.
 */
module_t *module_find(const char *name)
{
	for (module_t *curr = g_modules_list; curr; curr = curr->next)
	{
...
/**
 * @brief Check whether a module is already loaded.
 */
...
/**
 * @brief Set the name and path of the most recently loaded module.
 *
 * The most recently loaded module is always at the head of the list.
 */
...
/**
 * @brief Load a module from an in-memory ELF image.
 *
 * Validates the ELF header, maps allocatable sections, resolves symbols,
 * applies relocations, registers the module (making its exports available),
 * and runs its initialisation functions.
 *
 * Initialisation is attempted in this order:
 *   1. `.module.init` section (placed by module_init() macro)
 *   2. `.initcalls.*` sections (legacy kernel-style initcalls)
 *   3. A symbol named `module_init` (deprecated fallback)
 *
...
int module_load_buffer(const void *image, size_t image_size)
{
	...
	const char *shstrtab =
		(const char *)((uint8_t *)image + shstr_shdr->sh_offset);

	size_t section_count = ehdr->e_shnum;
	module_section_t *sections =
		kzalloc(section_count * sizeof(module_section_t));
	if (!sections)
		return -ENOMEM;
	...
	for (size_t i = 0; i < section_count; i++)
	{
		...
		if (shdr->sh_type == SHT_SYMTAB)
		{
			...
			strtab      = (const char *)
				     ((uint8_t *)image + linked->sh_offset);
		...
		uint64_t base = module_map_region(shdr->sh_size,
						  shdr->sh_flags,
						  shdr->sh_addralign);
		if (!base)
		{
	...
	if (!symtab || !strtab || strtab_size == 0)
	{
		kfree(sections);
		return -ENOEXEC;
	}
	...
	for (size_t i = 0; i < section_count; i++)
	{
		...
		if (shdr->sh_info >= section_count ||
		    !sections[shdr->sh_info].base)
			continue;
	...
	for (size_t i = 1;
	     i < sym_count && mod_export_count < MODULE_MAX_EXPORTS;
	     i++)
	{
	...
	/* ── Determine module base/extent and exit function ───────────────── */
	...
	memcpy(mod->exports, mod_exports,
	       sizeof(module_sym_export_t) * (size_t)mod_export_count);
	...
	for (size_t i = 0; i < section_count; i++)
	{
		...
		for (size_t j = 0; j < count; j++)
		{
			...
			if (ret != 0)
			{
				printk(KERN_ERR "[module] init failed: %d\n", ret);
	...
	for (size_t i = 0; i < sym_count; i++)
	{
		...
		if (ret != 0)
		{
			printk(KERN_ERR "[module] module_init symbol"
			       " failed: %d\n", ret);
			g_modules_list = mod->next;
			module_unmap_region(mod_base, mod_size);
			kfree(mod);
...
/**
 * @brief Load a module from a filesystem path.
 *
 * @param path Full path to the .ko file (e.g. "/modules/input.ko").
 * @return 0 on success, negative errno on failure.
 */
...
/**
 * @brief Load all .ko files from a directory.
 *
 * @param path Directory path (e.g. "/modules").
 * @return Number of modules loaded, or negative errno on failure.
 */
int module_load_directory(const char *path)
{
	...
	for (int i = 0; i < dir.count; i++)
	{
		...
		if (entry->name[0] == '.' ||
		    (entry->name[0] == '_' && strchr(entry->name, '~')))
			continue;
		...
		size_t full_len = path_len + (needs_slash ? 1 : 0) +
				  name_len + 1;
...
/**
 * @brief Unload a previously loaded module by name.
 *
 * Calls the module's exit function (if any), removes it from the
 * loaded-module list, unmaps its memory, and frees its bookkeeping
 * structure.
 *
...
/**
 * @brief Refresh module symbol tables (no-op in the current implementation).
 */
