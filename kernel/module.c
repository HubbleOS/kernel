...
static uint64_t module_resolve_symbol(const Elf64_Sym *sym,
                                      ...
                                      size_t section_count, const char *strtab,
                                      size_t strtab_size) {
 ...
 */
bool module_is_loaded(const char *name) { return module_find(name) != NULL; }
...
int module_load_buffer(const void *image, size_t image_size) {
  ...
  for (size_t i = 0; i < section_count; i++) {
    ...
    uint64_t base =
        module_map_region(shdr->sh_size, shdr->sh_flags, shdr->sh_addralign);
    if (!base) {
      kfree(sections);
      return -ENOMEM;
    }
  ...
  for (size_t i = 1; i < sym_count && mod_export_count < MODULE_MAX_EXPORTS;
       i++) {
  ...
  for (size_t i = 0; i < sym_count; i++) {
    ...
    uint64_t entry = module_resolve_symbol(&symtab[i], sections, section_count,
                                           strtab, strtab_size);
    if (entry == 0)
      break;
 ...
 */
int module_refresh_symbols(void) { return 0; }
